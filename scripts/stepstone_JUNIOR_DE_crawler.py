...
for seniority_level in seniority:
    ...
    while True:
        ...
        url =f"https://www.stepstone.de/5/job-search-simple.html?ke={seniority_level}%20&suid=325538cb-f6b6-4928-8a0f-72b106cd7cb8&se=15000&se=26000&se=23000&se=24000&se=17000&se=25000&se=16000&se=19001&se=31000&ob=date&action=facet_selected%3Bsectors%3B31000&of={page}"
        ...
        for i, link in enumerate(jobs):
            ...
            # driver.execute_script(f'''window.open("{link_to_ad_page}","_blank");''')
            ...
            time.sleep(8)
            random_scrolldown = np.random.randint(400, 800)
            ...
            while len(job_descri_elements) <1:
                ...
                if continue_scrape == 'n':
                    break
                job_descri_elements = driver.find_elements_by_css_selector('div[class="sc-jwKygS dfcwwF"]')
        ...
        print(f"Sleeping for {random_sleep + 60} seconds")
        time.sleep(60 + random_sleep)
    print('Starting next seniority level')
    print('_'*100)
